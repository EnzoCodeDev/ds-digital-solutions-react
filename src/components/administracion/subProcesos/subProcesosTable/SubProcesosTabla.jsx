...
import { SubProcesosTablaIndex } from "./SubProcesosTablaIndex";

export const SubProcesosTabla = () => {
  ...
  const handleTypeColumns = (rowInfo) => {
    if (rowInfo) {
      ...
      history.push(`/SubprocesosEdit/${uuid}`);
    }
  };
  const handleCreateSubProcess = () => {
    history.push(`/subProcesosCreate`);
  };
  return (
    <SubProcesosTablaIndex
      data={data}
      columns={columns}
      paginate={paginate}
      ...
      handleCreateSubProcess={handleCreateSubProcess}
    />
  );
};
